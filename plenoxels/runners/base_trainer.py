...
class BaseTrainer(abc.ABC):
    def __init__(self,
        ...
        self.depth_factor = kwargs.get('depth_factor')
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            ...
            depth_loss = self.criterion(fwd_out['depth'], data['depth_imgs'])
            depth_MRE = (torch.abs(fwd_out['depth'] - data['depth_imgs']) / data['depth_imgs']).mean()

            loss = recon_loss + self.depth_factor * depth_MRE
    ...
    def post_step(self, progress_bar):
        ...
        if self.save_every > -1 and self.global_step % self.save_every == 0:
            print()
            self.save_model()
            self.validate(show_training_renders=True)
    ...
    def _move_data_to_device(self, data):
        ...
        data["depth_imgs"] = data["depth_imgs"].to(self.device)
    ...
    def evaluate_metrics(self,
        ...
        def depth_MAE(depth_GT, out_depth):
            return np.abs(depth_GT- out_depth).mean()
        
        
        val_indices = self.val_indices
        train_indices = list(range(1, 51))
        for i in val_indices:
            train_indices.remove(i)

        if not train:
            depth_GT = torch.FloatTensor(depth_read(f"{depth_GT_path}/frame_{val_indices[img_idx]:04}.dpt")).unsqueeze(-1)
        else:
            depth_GT = torch.FloatTensor(depth_read(f"{depth_GT_path}/frame_{train_indices[img_idx]:04}.dpt")).unsqueeze(-1)

        mae = depth_MAE(depth_GT, out_depth)
        summary['test/depth_mae'] = mae

        out_depth = torch.cat((out_depth, depth_GT))
