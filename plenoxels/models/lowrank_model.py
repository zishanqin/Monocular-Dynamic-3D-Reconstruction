...
class LowrankModel(nn.Module):
    def __init__(self,
        ...
        self.field = KPlaneField(
            ...
            direction=kwargs['direction_config']
        )
    ...
    def forward(self, rays_o, rays_d, bg_color, near_far: torch.Tensor, timestamps=None):
        ...
        # print('Raysssssssss','')
        # print(len(ray_samples_list),ray_samples_list[0].origins.shape, ray_samples_list[0].directions.shape)  
        # print('---------', ray_samples_list[1])
        
        # import os
        # import torch

        # Assuming you have a list of torch arrays
        # for i in range
        # list_of_arrays = [ray_samples_list[0].origins, ray_samples_list[0].directions]
        # Each item represents different sampling layers from the proposal networks

        # Create a folder to save the arrays
        # folder_path = 'arrays_folder_cave'
        # os.makedirs(folder_path, exist_ok=True)

        # # Save each array in the list to separate files in the folder
        # names=['origin','directions']
        # import datetime

        # # Get the current time
        # current_time = datetime.datetime.now()
        # for i, array in enumerate(list_of_arrays):
        #     file_path = os.path.join(folder_path, f'{names[i]}_{current_time}.pt')
        #     torch.save(array, file_path)
        ...
        # # Assuming you have the origins and orientations as torch arrays in the shape of (4096, 1, 3)
        # origins = ray_samples_list[0].origins
        # orientations = ray_samples_list[0].directions

        # # Convert torch arrays to numpy arrays
        # origins_np = origins.squeeze().cpu().numpy()
        # orientations_np = orientations.squeeze().cpu().numpy()

        # # Create a 3D plot
        # fig = plt.figure()
        # ax = fig.add_subplot(111, projection='3d')

        # # Plot origins as blue points and orientations as red lines
        # ax.scatter(origins_np[:, 0], origins_np[:, 1], origins_np[:, 2], c='b', marker='o', label='Origins')
        # ax.quiver(origins_np[:, 0], origins_np[:, 1], origins_np[:, 2], orientations_np[:, 0], orientations_np[:, 1], orientations_np[:, 2], color='r', label='Orientations')
        ...
        # # Save the 3D plot as an image using PIL
        # plt.savefig('ray_samples_3d.png')

        # # Close the plot to free up memory
        # plt.close()

        # Optionally, you can also open and show the plot using:
        # plt.show()
