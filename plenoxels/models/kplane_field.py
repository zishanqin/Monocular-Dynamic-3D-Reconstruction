...
class KPlaneField(nn.Module):
    def __init__(
        ...
        if direction:
            if self.linear_decoder:
                ...
                self.in_dim_color = (
                        self.direction_encoder.n_output_dims  # removed for testing 
                        +
                        self.geo_feat_dim
            ...
            if self.linear_decoder:
                ...
                # This architecture is based on instant-NGP
                self.color_basis = tcnn.Network(
                    n_input_dims=self.appearance_embedding_dim, #self.direction_encoder.n_output_dims,
                    n_output_dims=3 * self.feature_dim,
                    network_config={
                        "otype": "FullyFusedMLP",
                        "activation": "ReLU",
    ...
    def forward(self,
        ...
        if self.direction: 
            # print("Direction on? ", self.direction)
            ...
            if self.linear_decoder:
                if self.use_appearance_embedding:
                    # basis_values = self.color_basis(torch.cat([embedded_appearance], dim=-1))
                    basis_values = self.color_basis(embedded_appearance)
                else:
                    basis_values = self.color_basis(torch.zeros_like(features))  # [batch, color_feature_len * 3]
                basis_values = basis_values.view(color_features.shape[0], 3, -1)  # [batch, 3, color_feature_len]
                rgb = torch.sum(color_features[:, None, :] * basis_values, dim=-1)  # [batch, 3]
                # rgb = rgb.to(directions)
                rgb = torch.sigmoid(rgb).view(n_rays, n_samples, 3)
            else:
                rgb = self.color_net(color_features).view(n_rays, n_samples, 3)
