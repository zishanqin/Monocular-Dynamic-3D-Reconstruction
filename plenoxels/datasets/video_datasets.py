...
class Video360Dataset(BaseDataset):
    ...
    def __init__(self,
        ...
        if "lego" in datadir or "dnerf" in datadir or "mpi" in datadir:
            dset_type = "synthetic"
        else:
            dset_type = "llff"
        ...
        if dset_type == "llff":
            ...
            if split == 'render':
                ...
                for scene in dnerf_durations.keys():
                    ...
                    if 'mpi' in datadir and scene in datadir:
                        num_tsteps = 50
            ...
            self.per_cam_near_fars = torch.tensor([[1.0, truncation]])
            ...
            if "dnerf" in datadir or "mpi" in datadir:
        ...
        # print("Check original boolean:", get_train_original)
        depth_imgs = []
        ...
        depth_GT_path = self.depth_gt_dir
        excluded_values = self.val_indices

        # Create a list from the range 1 to 50 excluding the specified values
        image_id = [i for i in range(1, 51) if i not in excluded_values]

        for idx in image_id:
            depth_GT = depth_read(f"{depth_GT_path}/frame_{idx:04}.dpt")
            depth_GT[depth_GT > truncation] = truncation
            depth_imgs.append(torch.from_numpy(depth_GT))
            

     
        depth_imgs = torch.stack(depth_imgs, 0)
        train_original = False
        if get_train_original:
            split = 'test' # temporally set split as test
            train_original = True
        ...
        if split == 'train':
            ...
            depth_imgs = depth_imgs.view(-1,1)

        elif imgs is not None:
            imgs = imgs.view(-1, intrinsics.height * intrinsics.width, imgs.shape[-1])
            depth_imgs = depth_imgs.view(-1, intrinsics.height * intrinsics.width, 1)
        ...
        if split == "train" and dset_type == 'llff':  # Only use importance sampling with DyNeRF videos
            ...
            if os.path.exists(os.path.join(datadir, f"ist_weights.pt")) and not train_original:
                ...
                if not train_original:
                    ...
                    t_e = time.time()
                    log.info(f"Computed {self.ist_wefights.shape[0]} IST weights in {t_e - t_s:.2f}s.")

        if self.isg and not train_original:
            self.enable_isg()
    ...
    def __getitem__(self, index):
        ...
        out = {
            ...
            "imgs": None,
            "depth_imgs": self.depth_imgs[index]
        }

        # print('depthimgs', out['depth_imgs'].shape)
        if self.split == 'train':
...
def load_llffvideo_poses(datadir: str,
    ...
    # print(poses.shape[0], len(videopaths))
